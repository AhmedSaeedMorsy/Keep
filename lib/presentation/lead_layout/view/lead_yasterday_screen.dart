...
class LeedYasterdayScreen extends StatelessWidget {
  const LeedYasterdayScreen({super.key});
      ...
      child: BlocBuilder<LeadsBloc, LeadsStates>(
        builder: (context, state) {
          return Padding(
            ...
            child: LeadsBloc.get(context).yasterdayLeads.isNotEmpty
                ? ListView.separated(
                    padding: EdgeInsets.zero,
                    physics: const BouncingScrollPhysics(),
                    itemBuilder: (context, index) => SharedWidget.leadItem(
                        context: context,
                        model: LeadsBloc.get(context).yasterdayLeads[index]),
                    separatorBuilder: (context, index) => SizedBox(
                      height: MediaQuery.of(context).size.height / AppSize.s32,
                    ),
                    itemCount: LeadsBloc.get(context).yasterdayLeads.length,
                  )
                : Center(
                    child: Text(
