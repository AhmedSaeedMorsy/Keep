...
class LeedAllScreen extends StatelessWidget {
  const LeedAllScreen({super.key});
      ...
      child: BlocBuilder<LeadsBloc, LeadsStates>(
        builder: (context, state) {
          return Padding(
            ...
            child: LeadsBloc.get(context).allAsignedLeads.isNotEmpty
                ? ListView.separated(
                    padding: EdgeInsets.zero,
                    physics: const BouncingScrollPhysics(),
                    itemBuilder: (context, index) => SharedWidget.leadItem(
                        context: context,
                        model: LeadsBloc.get(context).allAsignedLeads[index]),
                    separatorBuilder: (context, index) => SizedBox(
                      height: MediaQuery.of(context).size.height / AppSize.s32,
                    ),
                    itemCount: LeadsBloc.get(context).allAsignedLeads.length,
                  )
                : Center(
                    child: Text(
