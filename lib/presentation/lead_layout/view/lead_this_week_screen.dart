...
class LeedThisWeekScreen extends StatelessWidget {
  const LeedThisWeekScreen({super.key});
      ...
      child: BlocBuilder<LeadsBloc, LeadsStates>(
        builder: (context, state) {
          return Padding(
            ...
            child: LeadsBloc.get(context).thisWeekLeads.isNotEmpty
                ? ListView.separated(
                    padding: EdgeInsets.zero,
                    physics: const BouncingScrollPhysics(),
                    itemBuilder: (context, index) => SharedWidget.leadItem(
                        context: context,
                        model: LeadsBloc.get(context).thisWeekLeads[index]),
                    separatorBuilder: (context, index) => SizedBox(
                      height: MediaQuery.of(context).size.height / AppSize.s32,
                    ),
                    itemCount: LeadsBloc.get(context).thisWeekLeads.length,
                  )
                : Center(
                    child: Text(
